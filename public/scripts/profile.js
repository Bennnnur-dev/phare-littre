...
<article id="profile-info">
          <div id="profile-pic" style="background-color: ${data.color}" class="profile-pic"></div>
  <div>
 ...
 <button id="pin-button" data-state="${data.important && typeof data.important === "boolean" ? "PIN" : "NOPIN"}">
   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
    ...
    <div>${data.important && typeof data.important === "boolean" ? "Désépingler" : "Epingler"}</div>
</button>
    `;

  bodyContainer.innerHTML = `
<p class="body-info">Date de l'incident: <span id="span-date"></span></p>
<p id="body-desc">
  ...
  document.getElementById("body-desc").textContent = data.message;
  document.getElementById("span-date").textContent = data.date
    ? data.date.split("-").join("/")
    : "N/A";
  document.querySelector(".profile-pic").textContent =
    data.name[0].toUpperCase();
  document.querySelector(".profile-name").textContent = data.name.trim();
